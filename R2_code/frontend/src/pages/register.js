...
export default function Register(props) {
  ...
  // handle submit like checking pincode to be 7 digit, etc
  ...
  const handleSubmit = (event) => {
    ...
    if(data.tin[0].length!=11){
      alert("TIN is a 11-digit code");
      setButtonLoading(false)
      return;
    }

    if(data.gstNumber[0].length!=15){
      alert("GST number should be of 15 characters");
      setButtonLoading(false)
      return;
    }

    if(!data.emailId[0].match(mailformat)){
      alert("Invalid Email ID");
      setButtonLoading(false)
      return;
    }

    if(data.officePincode < 100000 || data.officePincode > 999999){
      alert("Pincode should be 6 digits");
      setButtonLoading(false);
      return;
    }

    if(data.phoneNumber[0].length!=10){
      alert("Enter a 10 digit number without the Country code");
      setButtonLoading(false)
      return;
    }
